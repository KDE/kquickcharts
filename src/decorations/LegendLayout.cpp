...
std::tuple<int, int, qreal, qreal> LegendLayout::determineColumns()
{
    ...
    // If we did not get a valid width for the entire layout, use maxWidth if
    // set or minWidth if not.
    ...
    auto itemWidth = fit ? (width() - m_horizontalSpacing * (columns - 1)) / columns : maxWidth;
